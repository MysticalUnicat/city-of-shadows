...
static int _load_key_comp(const void * _a, const void * _b, void * ud) {
...
static int _sort_files_comp(const void * _a, const void * _b, void * ud) {
...
struct Reader * vfs_read_from_pack(CStr path) {
  ...
  return read_from_pack(_source_files.data[pack_file->source_file], pack_file->offset, pack_file->length);
}

struct Reader * vfs_read(CStr path) {
  enum VFS_Namespace namespace = _default_namespace;
  if(string_compare_length(path, "pack://", 7) == 0) {
    namespace = VFS_Namespacee_PACK;
    path += 7;
  } else if(string_compare_length(path, "files://", 8) == 0) {
    namespace = VFS_Namespacee_FILES;
    path += 8;
  }
  switch(namespace) {
