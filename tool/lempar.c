...
** None.
*/
static void yyParseTrace(FILE *TraceFILE, char *zTracePrompt){
...
*/
static void yyParseInit(void *yypRawParser ParseCTX_PDECL){
...
*/
static void *yyParseAlloc(void *(*mallocProc)(YYMALLOCARGTYPE) ParseCTX_PDECL){
  yyParser *yypParser;
  yypParser = (yyParser*)(*mallocProc)( (YYMALLOCARGTYPE)sizeof(yyParser) );
  if( yypParser ){
    ParseCTX_STORE
    yyParseInit(yypParser ParseCTX_PARAM);
  }
...
*/
static void yyParseFinalize(void *p){
...
*/
#ifdef YYTRACKMAXSTACKDEPTH
static int yyParseStackPeak(void *p){
  yyParser *pParser = (yyParser*)p;
...
*/
#if defined(YYCOVERAGE)
static int yyParseCoverage(FILE *out){
...
*/
static int yyParseFallback(int iToken){
#ifdef YYFALLBACK
...
static int Parse_lex(struct Parse * parser) {
  // hack, darn thing did not detect $ properly
  if(parser->cursor < parser->limit && *parser->cursor == 0 && parser->state == -1) {
    parser->cursor++;
    return END_OF_INPUT;
  }
  /*!getstate:re2c*/
  /*!re2c
    ...
    re2c:define:YYCURSOR   = "parser->cursor";
    re2c:define:YYMARKER   = "parser->marker";
    re2c:define:YYLIMIT    = "parser->limit";
    re2c:define:YYGETSTATE = "parser->state";
    re2c:define:YYSETSTATE = "parser->state = @@;";
    re2c:variable:yyaccept = "parser->yyaccept";
    re2c:variable:yych     = "parser->yych";
...
void Parse_add_buffer(void * _parser, const uint8_t * data, uint64_t size) {
  ...
  for(;;) {
    ...
    if(parser->previous != NULL) {
      token.string = format_alloc("%s%.*s", parser->previous, length, base);
      string_free(parser->previous);
      parser->previous = NULL;
    } else {
      token.string = string_clone_length(base, length);
    }
    // TRACE(SOURCE_NAMESPACE, "token %i %s %i \"%s\"", token.code, yyTokenName[token.code], string_size(token.string), token.string);
    if(token.code == WAITING) {
      if(parser->previous != NULL) {
        parser->previous = string_concatenate(parser->previous, token.string);
      } else {
        parser->previous = string_clone(token.string);
      }
      break;
    }
...
void * Parse_finish(void * _parser) {
  ...
  parser->is_finalized = true;
#if defined(PARSER_EXTRA_DESTRUCTOR)
  struct Extra * extra = &parser->extra;
...
void Parse_free(void * _parser) {
  ...
  memory_free(_parser, sizeof(struct Parse), alignof(struct Parse));
}
